...
export function Analysis() {
  ...
  const pollResult = (id: string) => {
    const poll = async () => {
      try {
        const res = await api.getAnalysisResult(id) as { status: string; data?: AnalysisData; error?: string; progress?: ProgressInfo };
            ...
            {progressInfo ? (
              <>
                <Progress
                  ...
                  label={`${progressInfo.current} / ${progressInfo.total}`}
                />
                <TextContent style={{ marginTop: 8 }}>
                  <Text>{progressInfo.message}</Text>
                </TextContent>
              </>
            ) : (
              <>
        ...
        <>
          ...
          {jobId && (
            <Split hasGutter style={{ marginBottom: 16 }}>
              <SplitItem>
                <Button variant="secondary" component="a" href={api.exportAnalysisJson(jobId)} target="_blank">
                  Download JSON
                </Button>
              </SplitItem>
              <SplitItem>
                <Button variant="secondary" component="a" href={api.exportAnalysisHtml(jobId)} target="_blank">
          ...
          <Tabs activeKey={activeTab} onSelect={(_e, k) => setActiveTab(k as number)}>
            ...
            {/* Tab 2: Organizations (with drill-down) */}
