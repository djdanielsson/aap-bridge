    ...
    <div className="mpv-dist-bar">
      ...
      {pctSkipped > 0 && (
        <Tooltip content={`${migration.totalSkipped} skipped`}>
          <div
            className="mpv-dist-bar__segment mpv-dist-bar__segment--skipped"
      ...
      {pctFailed > 0 && (
        <Tooltip content={`${migration.totalFailed} failed`}>
          <div
            className="mpv-dist-bar__segment mpv-dist-bar__segment--failed"
...
function MigrationOutputToolbar({
  allExpanded,
  onExpandAll,
  onCollapseAll,
  onScrollTop,
...
function ResourceRow({ item }: { item: ResourceItem }) {
  ...
  return (
    <div className={`mpv-resource-row mpv-resource-row--${item.result}`}>
      <div className="mpv-resource-row__icon">
        <ResourceResultIcon result={item.result} />
      </div>
      ...
      {item.detail && (
        <div className="mpv-resource-row__detail">{item.detail}</div>
      )}
    </div>
  );
}
...
function MigrationPhaseGroup({
  ...
  return (
    <div className={`mpv-phase-group mpv-phase-group--${phase.status}`}>
      <div
        className={`mpv-phase-group__header${hasContent ? ' mpv-phase-group__header--clickable' : ''}`}
        onClick={hasContent ? onToggle : undefined}
      >
        <div className="mpv-phase-group__gutter">
          [{phase.num}/{totalPhases}]
        </div>
        <div className="mpv-phase-group__toggle">
          {hasContent ? (
            ...
            <span className="mpv-phase-group__toggle-spacer" />
          )}
        </div>
        <div className="mpv-phase-group__icon">
        ...
        <div className="mpv-phase-group__duration">
          {phase.duration || (phase.status === 'running' ? phase.elapsed : '')}
        </div>
      </div>
        ...
        <div className="mpv-phase-group__body">
          {phase.status === 'running' && phase.exported > 0 && (
            <div className="mpv-phase-group__progress">
              ...
              />
              <span className="mpv-phase-group__progress-text">
                {phaseTotal}/{phase.exported} &middot; {phase.rate}
              </span>
            </div>
          )}
          ...
          {phase.error && (
            <div
              className="mpv-item-row mpv-item-row--error mpv-item-row--clickable"
              onClick={() => onErrorClick(phase.error!)}
              role="button"
              tabIndex={0}
            >
              <div className="mpv-item-row__gutter">
                <ExclamationCircleIcon className="mpv-icon--danger" />
              </div>
...
export function MigrationProgressView({ events, jobStatus }: Props) {
  ...
  const allExpanded =
    migration.phases.length > 0 && migration.phases.every((p) => expanded[p.num]);
      ...
      <div className="mpv-scroll" ref={scrollRef}>
        {migration.phases.map((phase) => (
          <MigrationPhaseGroup
            ...
            onToggle={() => toggleExpand(phase.num)}
